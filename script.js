...
class AIChat {
    ...
    async findAnswer(question) {
        ...
        // Простые однозначные ответы
        if (lowerQuestion === 'да' || lowerQuestion === 'yes') {
            return "Отлично! О чём конкретно хотите узнать? Могу рассказать о проектах, опыте работы или увлечениях Сергея 🎯";
        }
        
        if (lowerQuestion === 'нет' || lowerQuestion === 'no') {
            return "Понял! Если передумаете - спрашивайте. Всегда рад рассказать о Сергее и его творчестве 😊";
        }
        
        if (lowerQuestion === 'ок' || lowerQuestion === 'окей' || lowerQuestion === 'хорошо') {
            return "Супер! Что вас интересует? Проекты, опыт или может философия Сергея? 💫";
        }
        
        if (lowerQuestion === 'здорово' || lowerQuestion === 'круто' || lowerQuestion === 'класс') {
            return "Согласен! Сергей действительно крутой специалист. Хотите узнать о его проектах подробнее? 🚀";
        }

        // Проверяем общие вопросы для болтовни
        if (lowerQuestion.includes('привет') || lowerQuestion.includes('здравств') || 
            lowerQuestion === 'hi' || lowerQuestion === 'hello' || lowerQuestion === 'хай') {
            return this.getRandomResponse(knowledgeBase.привет);
        }
        
        if (lowerQuestion.includes('как дел') || lowerQuestion.includes('как сам') || 
            lowerQuestion.includes('настроен') || lowerQuestion.includes('как настроен') ||
            lowerQuestion.includes('настроение')) {
        ...
        // Обработка критики и негатива
        if (lowerQuestion.includes('туп') || lowerQuestion.includes('глуп') || 
            lowerQuestion.includes('дурак') || lowerQuestion.includes('плох')) {
            return "Простите, я только учусь! Сергей говорит, что важно сохранять терпение. Может, спросите что-то конкретное о его работе? 🤗";
        }

        if (lowerQuestion.includes('умеешь') || lowerQuestion.includes('можешь') || lowerQuestion.includes('способн')) {
            return "Я умею рассказывать о Сергее Канчере: его проектах, опыте, философии и увлечениях. Что именно вас интересует? 😊";
        }
